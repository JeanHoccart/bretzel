...
- ``valid``     : composed over ``root`` while a drag is in flight and
                  this zone would accept it. Requirement 1 of the cadrage
                  — « surlignage des zones valides ».
...
DROPZONE_THEME: dict[str, Any] = {
    "slots": {
        ...
        # ``data-[bz-drop-replace]`` : posé par le geste sur une zone
        # ``holds="one"`` qui porte DÉJÀ un occupant. Elle ne reçoit pas
        # le nœud — l'y glisser lui en donnerait deux — donc il faut que
        # l'œil comprenne qu'elle sera ÉCRASÉE, pas complétée.
        #
        # Un trait plein et une teinte, là où l'acceptation ordinaire est
        # en pointillés : deux états voisins doivent se distinguer sans
        # qu'on les compare côte à côte.
        "replace": (
